...
# This script is executed when a subagent completes.
# Return Decision = Block to prevent the subagent from stopping (e.g., to verify its results first).
# Always check StopHookActive to prevent the subagent from running indefinitely.
...
try {
    ...
    $in = [SubagentStopInput]::FromJson($json)
    $out = [SubagentStopOutput]::new()
